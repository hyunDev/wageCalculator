...
export default function App() {
  ...
  const calculateWage = useCallback(() => {
    setEndTime(getCurTime());
    const _wage = (endTime - startTime - pausedTime) * (increment / 3600);
    setWage(_wage);
    setEndFormatTime(getFormatTime(endTime));
  }, [endTime, increment, pausedTime, startTime]);

  const syncFormatTime = useCallback(() => {
    if (startTime === 0) {
      setStartFormatTime('0');
    } else {
      console.log('sync startTime ::', startTime);
  ...
  useEffect(() => {
    ...
    } else if (startIncrementing === false && startTime !== 0) {
      clearInterval(intervalStartId);
      intervalPauseId = setInterval(() => {
        setPausedTime(pausedTime + 1);
      }, 1000);
    }
  }, [calculateWage, pausedTime, startIncrementing, startTime, syncFormatTime]);
