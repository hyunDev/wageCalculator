...
export default function TimerScreen({}) {
  ...
  const calculateWage = useCallback(() => {
    ...
    const _currentEndTime = Math.floor(currentEndTime.getTime() / 1000);
    const _startTime = Math.floor(startTime.getTime() / 1000);
    const _wage =
      (_currentEndTime - _startTime - pausedTime) * (increment / 3600);
    setWage(_wage);
    setEndFormatTime(getFormatTime(currentEndTime));
